...
public abstract class AbstractGenericException extends Exception{
     ...
     * Causa do erro
     * @param parameters
     * parametros para a mensagem de usuario no padrao formatmsg
     * @author A0089321 - Arthur Gomes
     */
        ...
        if (innerCause instanceof SQLException) {
            ...
            try {
                ...
                ResourceBundle bundle = ResourceBundle.getBundle(
                        MessagesEnun.EXCEPTION_ARCHIVE_MESSAGES.getValue(), Locale.getDefault(), this
                ...
                if (mensagemBundle != null) {
                    // TODO MessageFormat is not synchronized (preciso
    ...
    /**
     * todas as exceptions dos sistemas deve utilizar este construtor onde o
     * tipo de mensagem indica se é um erro de sistema ex: falha ao conectar na
     * base de dados ou um erro de negócio. Ex período inválido. a mensagem deve
     * ser uma constantes para a mensagem que estará no bundle do struts.
     *
     * @param tipoMensagem
     * tipo da mensagem: erro de sietema ou erro de negocio
     * @param mensagemAmigavel
     * mensagem que aparecerá para o usuário
     * @param parametrosMensagem
     * parametros da mensagem amigavel
     * @param stackTrace
     * stackTrace do erro
     */
     ...
     *
     * @param paramMensagem
     *  parametros para a mensagem de negocio
     */
    ...
    /**
     * Procura a mensagem no bundle de exception caso nao existe repassa a
     * mensagem.
     *
     * @param msgErro
     *            mensgem de negocio
     ...
     *
     * @param msgErro
     *  constante de msg de erro a ser internacionalizada *
    ...
    private String getMensagemInternacionalizada(String msgErro) {
        ...
        try {
            bundle = ResourceBundle.getBundle(MessagesEnun.EXCEPTION_ARCHIVE_MESSAGES.getValue(), Locale
                    .getDefault(), this.getClass().getClassLoader());
        } catch (Exception e1) {
            logger.error("erro ao acessar o bundle de exeption " + e1.getMessage());
            logger.error("O projeto deve possuir um bundle de msg de erro");

        }
        // se o bundle nao foi carregado retorna a mensagem de erro
        // senao le a msg do resource bundle
        if (bundle == null) {
            return msgErro;
        } else {
            String mensagemInternacionalizada = msgErro;

            if (StringUtils.isNotBlank(msgErro)) {
                msgErro = msgErro.replaceAll("\n", "");
                msgErro = msgErro.replaceAll(" ", "_").trim();
                // tratamentos para mensagens de erro do hibernate
                // hold.adriano
                String[] msgErroTmp;
                msgErroTmp = msgErro.split("_\\[");
                msgErro = msgErroTmp[0].toString();

                msgErroTmp = msgErro.split(":_br");
                msgErro = msgErroTmp[0].toString();
            }

            try {
                mensagemInternacionalizada = bundle.getString(msgErro);
                if (this.parametrosMensagem != null) {
                    mensagemInternacionalizada = MessageFormat.format(mensagemInternacionalizada,
                            this.parametrosMensagem);
                }
            } catch (MissingResourceException e) {

                logger.debug("chave não existe no bundle de exception");
            }
            return mensagemInternacionalizada;

        }
    }
   
    /**
     * Método utilitário utilizado na infra de exceção para transformar uma lista de exceção na representação de String separando as mensagens
     * pelo caracter "|". <br /><br />
     *
     * O resultado desse retorno, será utilizado para iterar nas mensagens da tela do usuário.
     *
    ...
    /**
     * @param mensagemStackTrace
     *            The mensagemStackTrace to set.
     */
